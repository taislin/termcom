...
### Браузерная версия (WASM)

> [!NOTE]
> Версия WASM отображается нативно в браузере без серверного приложения Go.

Версия WASM компилирует ядро игры на Go в WebAssembly и отображает напрямую в браузере через Canvas-рендерер с поддержкой истинных цветов ANSI.

**Быстрый старт:**

```bash
# Собрать WASM бинарник
cd cmd/termcom_wasm
...
**Возможности:**
- Нативная отрисовка в браузере (без серверного приложения)
- Символьная сетка Canvas с поддержкой истинных цветов ANSI
- Дифференциальная отрисовка (перерисовываются только изменённые ячейки)
- Эффект тряски экрана через CSS-трансформации
- Автоматический размер шрифта (переопределите параметром URL `?font=ИмяШрифта`)
- Поддержка сенсорного ввода на мобильных (нажмите для клика, долгое нажатие для правой кнопки, перетаскивание для прокрутки)
