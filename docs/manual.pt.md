# termcom — Manual do Demake Roguelike em ASCII X-COM (v0.50.5)
...
16. [Tabelas de Referência](#tabelas-de-referência)
...
| Tipo | Char | Cobertura | Notas |
|------|------|-------|-------|
| Chão | `.` | 0% | Terreno padrão |
| Parede | `#` | 80% | Bloqueia movimento e LdS |
| Porta | `+` | 0% | Abre ao contacto |
| Janela | `¤` | 0% | Bloqueia LdS, transponível |
| Relva | `·` | 0% | Inflamável |
| Árvore | `♣` | 60% | Bloqueia LdS, inflamável |
| Rocha | `∩` | 70% | Bloqueia LdS |
| Água | `≈` | 0% | Intransponível |
| Chão OVNI | `≡` | 0% | Pavimento interior |
| Parede OVNI | `█` | 80% | Bloqueia movimento e LdS |
| Escadas | `▓` / `▒` | 0% | Transição de nível |
| Pavimento | `░` | 0% | Estrada / plataforma de aterragem |
| Areia | `·` | 0% | Terreno desértico |
| Neve | `∗` | 0% | Terreno polar |
| Pântano | `≋` | 0% | Terreno de pântano |
| Arbusto | `†` | 40% | Cobertura leve, inflamável |
| Cerca | `║` | 30% | Cobertura mínima, inflamável |
| Entulho | `▒` | 20% | Terreno destruído |
| Mobília | `■` `⚙` `◈` `⌁` `▤` `⊕` `⎔` `⊟` `⌨` `□` `◫` `⊞` | 50% | Objetos de OVNI/prédio |

### Tipos de Dano

| Tipo | Fonte |
|------|--------|
| Cinético | Pistola, Rifle, Canhão Pesado, Canhão Automático |
| Laser | Pistola Laser, Rifle Laser |
| Plasma | Pistola de Plasma, Rifle de Plasma, Plasma Pesado, Granada Alien |
| Explosivo | Lançador de Foguetes, Granadas, Minas |
| Corpo a Corpo | Cajado de Atordoamento, Garra Chryssalid, Garra Reaper |
| Psiónico | Ataques psi Ethereal |

### Terreno Destruível

| Tile | Torna-se | Mudança de cobertura |
|------|---------|-------------|
| Parede / Parede OVNI | Entulho | 80% → 20% |
| Árvore | Entulho | 60% → 20% |
| Rocha | Entulho | 70% → 20% |
| Cerca | Entulho | 30% → 20% |

### Gás (de granadas)

| Densidade | Visual | Bloqueia LdS? | Penalização de cobertura |
|---------|--------|-------------|---------------|
| Denso (3) | ▓ | Sim | 40% |
| Médio (2) | ▒ | Não | 20% |
| Fino (1) | ░ | Não | 0% |

O gás espalha-se para tiles adjacentes e dilui a cada turno até dissipar.
...
| Propagação | Hipótese por turno para tile inflamável adjacente |
| Inflamável | Relva, Árvore, Arbusto, Cerca, Porta |
| Duração | ~3 turnos, depois o tile torna-se Chão |

### Fabrico

| Item | Tempo | Materiais |
|------|------|-----------|
| Pistola | 3 dias | 1 liga |
| Rifle | 5 dias | 2 ligas |
| Canhão Pesado | 7 dias | 3 ligas |
| Canhão Automático | 6 dias | 3 ligas |
| Lançador de Foguetes | 8 dias | 4 ligas, 1 elério |
| Cajado de Atordoamento | 2 dias | 1 liga |
| Armadura Pessoal | 6 dias | 2 ligas |
| Fato Leve | 10 dias | 4 ligas, 1 elério |
| Fato Médio | 14 dias | 6 ligas, 2 elério |
| Fato Pesado | 18 dias | 8 ligas, 3 elério |
| Kit Médico | 3 dias | 1 liga |
